...
OPENAI_BASE_URL = os.environ.get("OPENAI_BASE_URL", "https://api.smol.ai")
...
def inject_context():
    ...
    # if filename ends with csv, use pd.read_csv to read the file and get the headers
    if filename.endswith('.csv'):
        import pandas as pd
        df = pd.read_csv(os.path.join(os.getcwd(), 'workspace', filename))
        headers = df.columns.tolist()
        message_buffer.append(""" CSV uploaded. Headers are: """ + str(headers) + """\n\n""")
